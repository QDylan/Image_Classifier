...
为了在多个GPU或多个机器上轻松训练模型，
该模块提供了一组辅助函数：`create_clones`，`optimize_clones`和`deploy`。
...
class DeploymentConfig(object):
  """Configuration for deploying a model with `deploy()`.
	使用此类的实例传递给deploy()以指定如何部署要构建的模型
	如果未传递，则将使用从默认deployment_hparams构建的实例
  ...
  def __init__(self,
	...
	若num_replicas=1，该模型通过单个流程进行部署，
	此时忽略 `worker_device`, `num_ps_tasks`, and `ps_device`
	...
	若num_replicas比1大，则`worker_device`&`ps_device`必须为`worker`&`ps`指定TF设备
	`num_ps_tasks`必须为正
    ...
    self._num_clones = num_clones		# 1
    self._clone_on_cpu = clone_on_cpu	# False
    self._replica_id = replica_id		# 0
    self._num_replicas = num_replicas	# 1
    self._num_ps_tasks = num_ps_tasks 	# 0
	# '/job:'
  ...
  def variables_device(self):
	...
	返回用在克隆内创建的变量的设备，适合tf.device()的值
    Returns:
      A value suitable for `tf.device()`.
    """
    device = ''
    if self._num_ps_tasks > 0:  #0
    ...
    class _PSDeviceChooser(object):
      ...
      def __init__(self, device, tasks):
        self._device = device # ='/device:CPU:0'
        self._tasks = tasks
        self._task = 0
